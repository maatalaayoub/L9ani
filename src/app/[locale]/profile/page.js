...
export default function ProfilePage() {
    ...
    const handleSendVerificationEmail = async () => {
        ...
        try {
            ...
            if (result.alreadyVerified) {
                // Email is already verified, reload the page to update UI
                window.location.reload();
                return;
            }
            ...
            // Show success message
            setVerificationMessage(t('verify.emailSent') || 'Verification email sent! Check your inbox.');
            setTimeout(() => setVerificationMessage(''), 5000);

        } catch (err) {
            console.error('Send verification email error:', err);
            setVerificationError(err.message || t('errors.verificationFailed'));
    ...
    const handleForgotPassword = async () => {
        ...
        try {
            // Use our custom API (Resend) instead of Supabase's built-in
                ...
                <div className={`relative rounded-xl border shadow-sm overflow-hidden ${
                    ...
                    <div className="relative p-6 sm:p-8">
                        <div className="flex flex-col sm:flex-row items-center sm:items-start gap-5">
                            ...
                            {/* Action Buttons */}
                            <div className="flex-shrink-0 mt-2 sm:mt-0 flex flex-col sm:flex-row gap-2">
                                {/* Admin Dashboard Button - Only visible to admins */}
                                {isAdmin && !isEditing && (
                                    <Link
                                        href="/admin"
                                        className="inline-flex items-center justify-center gap-2 px-4 py-2 text-sm font-medium text-white bg-gradient-to-r from-amber-500 to-orange-500 rounded-lg hover:from-amber-600 hover:to-orange-600 transition-all shadow-sm"
                                    >
                            ...
                            <div className="space-y-4">
                                ...
                                <div className="flex justify-between items-center py-2 border-b border-gray-100 dark:border-gray-800 gap-4">
                                    ...
                                    <div className="flex items-center justify-end gap-2 flex-1 min-w-0">
                                        ...
                                        {/* Verify Email Button - sends verification link via email */}
                                        {!profile?.email_verified && !isOAuthUser && (
                                            <button
                                                onClick={handleSendVerificationEmail}
                                                disabled={isSendingVerification}
                                                className="px-2 py-0.5 bg-yellow-100 dark:bg-yellow-900/30 text-yellow-700 dark:text-yellow-400 text-xs rounded border border-yellow-200 dark:border-yellow-800 hover:bg-yellow-200 dark:hover:bg-yellow-900/50 transition-colors shrink-0 disabled:opacity-50"
                                            >
                                                {isSendingVerification ? t('verify.sending') || 'Sending...' : t('verify.button')}
                                            </button>
                                        )}
                                        <button
                                            onClick={() => {
