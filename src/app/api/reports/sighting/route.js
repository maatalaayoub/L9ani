    ...
    try {
        ...
        if (hasPhotoChanges && existingReport.report_type === 'person' && finalPhotoUrls.length > 0) {
            ...
            try {
                ...
                faceRecognitionResult = await processFaceRecognition(reportId, 'sighting', finalPhotoUrls, user.id);
...
export async function DELETE(request) {
    try {
        ...
        if (commentsError) {
            console.error('[API Sighting Reports DELETE] Error deleting comments:', commentsError);
        } else {
            console.log('[API Sighting Reports DELETE] Deleted associated comments');
        }
        ...
        const { error: reactionsError } = await supabaseAdmin
            ...
            .eq('sighting_report_id', reportId);
        if (reactionsError) {
            console.error('[API Sighting Reports DELETE] Error deleting reactions:', reactionsError);
        } else {
            console.log('[API Sighting Reports DELETE] Deleted associated reactions');
        }
        ...
        if (notificationsError) {
            console.error('[API Sighting Reports DELETE] Error deleting notifications:', notificationsError);
        } else {
            console.log('[API Sighting Reports DELETE] Deleted associated notifications');
        }
