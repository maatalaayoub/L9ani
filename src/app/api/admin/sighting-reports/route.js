...
// Batch helper to get details for multiple sighting reports at once (avoids N+1 queries)
async function getBatchSightingReportDetails(reports) {
    ...
    const fetchPromises = Object.entries(reportsByType).map(async ([type, reportIds]) => {
        const tableName = SIGHTING_DETAIL_TABLE_MAP[type];
        if (!tableName) return;
        ...
        try {
            ...
            console.error(`[Admin Sighting Reports] Batch fetch error for ${tableName}:`, err);
        }
    });
...
async function getBatchSightingProfiles(userIds) {
    ...
    try {
        ...
        if (error) {
            console.error('[Admin Sighting Reports] Batch profiles fetch error:', error);
            return {};
        }
        ...
        console.error('[Admin Sighting Reports] Batch profiles exception:', err);
        return {};
    }
}
...
export async function GET(request) {
    try {
        ...
        const [detailsMap, profilesMap] = await Promise.all([
            getBatchSightingReportDetails(reportsData || []),
            getBatchSightingProfiles((reportsData || []).map(r => r.user_id))
        ]);
    ...
    try {
        ...
        if (action === 'reject') {
            try {
                ...
                console.log('[API Admin Sighting Reports PATCH] Face cleanup result:', faceCleanupResult);
            } catch (faceErr) {
                console.error('[API Admin Sighting Reports PATCH] Error cleaning up faces:', faceErr);
